...
impl<'a> FnCompiler<'a> {
    ...
    fn check_unused_local(&mut self, local: &crate::scope::Local) {
        ...
        if local.name == "self" && local.is_implicit_self {
            return;
        }
    ...
    fn is_package_path_target(var: &ast::Var) -> bool {
        ...
        match ve.prefix() {
            ast::Prefix::Name(tok) if tok_str(tok) == "package" => {}
            _ => return false,
        }
    ...
    fn try_eval_static_string(&self, expr: &ast::Expression) -> Option<String> {
        match expr {
            ...
            ast::Expression::Var(ast::Var::Expression(ve)) => {
                ...
                match &suffixes[0] {
                    ast::Suffix::Index(ast::Index::Dot { name, .. }) if tok_str(name) == "path" => {
...
pub(crate) fn extract_require_literal(expr: &ast::Expression) -> Option<String> {
    ...
    match fc.prefix() {
        ast::Prefix::Name(tok) if tok_str(tok) == "require" => {}
        _ => return None,
    }
